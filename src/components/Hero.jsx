...
export default function Hero({ t }) {
  return (
    <header id="inicio" className="scroll-mt-32 relative px-6 py-16 md:py-24 max-w-7xl mx-auto flex items-center min-h-[70vh] overflow-hidden">
        
        {/* BÚHO DE FONDO — misma capa para todos los tamaños */}
        <motion.div 
            animate={{ y: [0, -15, 0] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            className="absolute inset-0 lg:inset-auto lg:right-0 lg:top-0 lg:bottom-0 lg:w-[105%] z-0 pointer-events-none"
        >
            <div className="relative w-full h-full lg:top-12">
                {/* Gradiente izquierdo: full en móvil, 2/3 en escritorio */}
                <div className="absolute inset-y-0 left-0 w-full lg:w-2/3 bg-gradient-to-r from-[#060413] via-[#060413]/90 to-transparent z-10" />
                <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-[#060413] to-transparent z-10" />
                <img 
                    src="/buho-hero.png.jpeg" 
                    alt="Búho" 
                    className="object-contain w-full h-full object-right-bottom drop-shadow-2xl opacity-40 lg:opacity-100" 
                />
            </div>
        </motion.div>
        ...
        <div className="relative z-20 w-full lg:max-w-xl">
            <motion.h1 
                ...
                className="font-syne text-[2.5rem] max-sm:text-4xl sm:text-7xl font-extrabold uppercase tracking-tighter leading-[0.9] mb-6"
            >
