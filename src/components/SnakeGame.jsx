...
function SnakeGame({ onExit }) {
  ...
  useEffect(() => {
    const s = 'sm';
    const cols = Math.floor(SIZES[s].w / SIZES[s].cell);
    const rows = Math.floor(SIZES[s].h / SIZES[s].cell);
    const cx = Math.floor(cols / 2), cy = Math.floor(rows / 2);
    const body = [{ x: cx, y: cy }, { x: cx - 1, y: cy }, { x: cx - 2, y: cy }];
    stateRef.current.snake = body;
    stateRef.current.dir = { x: 1, y: 0 };
    stateRef.current.nextDir = { x: 1, y: 0 };
    stateRef.current.score = 0;
    stateRef.current.food = placeFood(body, s);
    stateRef.current.size = s;
    drawFrame();
    return () => stopLoop();
  }, [placeFood, drawFrame, stopLoop]);
